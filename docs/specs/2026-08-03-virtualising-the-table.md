...
figures *start*, and dumping the excess in the name column starts them **160.70px**
further right — the committed fleet at `msrpGbp,weight,plate` on a 1,146px track,
755.30px of name column against 916.00px. `lib/fit.ts`'s `columnWidths` docblock
is the one home for that figure and its conditions.
