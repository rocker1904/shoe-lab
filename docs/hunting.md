...
| `hunt/fit-boundary.mjs` | **asserts**, and exits non-zero: the sidebar's 1180px boundary against the real fleet |
...
names whose document fits at every width, so no assertion in the suite can see the overflow that
1180 is derived from (docs/app.md §Filters). Run it when the fleet grows, when `defaultColumns`
changes, or when a table cell's wording moves — those are the three inputs that move the boundary,
and each moves it silently.
