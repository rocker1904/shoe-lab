...
**The columns are declared, not derived.** The desktop table is
`table-layout: fixed` over a `<colgroup>` the model writes: `columnWidths` in
`lib/fit.ts` shares the table's track between `['name', ...cols]` and
`ShoeTable.svelte` declares the result. So a column's width is a function of the
fleet and the track and of nothing else — where auto layout made it a function of
**the rows that happen to be in the DOM**, which is a property no table can keep
once it renders a window of its fleet rather than all of it
(docs/specs/2026-08-03-virtualising-the-table.md §Decisions). It reproduces what
auto layout drew: measured on the real fleet at five widths in two engines, no
column moves more than **1.6px** in Chromium or **9.3px** in Firefox at 2560px,
and the table's own width, the header band and the row height are identical to
the pixel.

The track is **measured off the panel** rather than computed from the window.
`fit.ts` answers what the panel has to be *given* — a question about gutters and
the sidebar, answerable before anything is mounted — and `.tblwrap`'s own
`clientWidth` answers what the table was actually handed; the declared widths
have to sum to the second or fixed layout redistributes the difference and the
rule that shared them is not the one on screen. Before the first measurement, and
under jsdom for ever, every column takes its own minimum, which is the same
...
hidden container at the width it wraps at, and its own **box** is read back —
never a line count, which would have to be turned back into a height by a rule
about what a line is worth, and that rule is face-dependent as well as
engine-dependent. The container is **cloned from a live cell**, which is what
...
- **A natural hyphen is never a modelled break opportunity**, so an upstream
  label or shoe name contributes its whole hyphenated token. It is deliberate
  ...
  **under** the other's min-content, which is a header or name hanging out of a
  declared column.
  ...
  **That is an upper bound only if the split is on separators every engine breaks
  ...
  a **space, a tab or a line feed** and nowhere else in unmarked input, and the
  set is measured in the three engines rather than named — `lib/labels.test.ts`
  pins one of each class that is out, over the fifty-odd characters the closure
  argument runs through. A dropped catalogue slug is the one authored exception: `columnLabel`
  inserts U+200B after each visible hyphen, and `wordsOf` splits at that marker.
  The three engines therefore receive the same explicit opportunity the model
  reads, without changing the slug's visible text; the phone table and all three
  proportional desktop font tables price the marker at its measured zero
  advance. The measurement rig retains it on regeneration.
  ...
  **6.42px** at the name face, the worst reading taken anywhere, and it needs no
  ...
  `FIT_DROPPED_COLS` in `app/e2e/fit-support.ts` includes the worst chunking the
  URL's 64-character slug grammar accepts, so the three-engine agreement and
  declared-column guards quantify over the parser's edge rather than today's
  catalogue. `lib/fit.test.ts` pins that witness on both sides of the length door,
  its modelled width and the marker's zero residual; a font-table or grammar
  change therefore requires the bound to be re-read rather than silently
  restoring the old surplus width.
- The **name column's floor** is the wider of `NAME_COL_PX` — the model's own
  `14rem`, and no longer a cell's `min-width`, which a declared width makes inert
  — and the fleet's own longest unbreakable token. The `discontinued` chip counts with
...
three kinds of input it keeps apart: the tokens (the `--s2` cell padding,
`--caret-w`, the panel's own borders — plus the 14rem name column, which the
model now *declares* rather than transcribes, a declared column width having made
the cell floor it used to read inert), committed
...
four column sets, to a stated 4px tolerance. **The declared widths come off
first**, or the question is not the engine's to answer: a `fixed` table asked for
its `min-content` replies with the model's own arithmetic, and the assertion
would compare the model to itself. `measureFit` clears the `<colgroup>`, restores
`table-layout: auto`, and hands the engine `NAME_COL_PX` — the one input that is
a declared design floor rather than something a string implies
(§Table presentation). The measured disagreement is at most
...
asked about. A pointer-transparent geometric hover corridor spans the placement
gap on either side of the panel, so pointer speed does not determine whether its
source link remains reachable and a press in that visually outside space still
reaches the control beneath it. Focus is retained rather than dropped, and outside
press, Escape and focus departure remain the shared closing contract.
