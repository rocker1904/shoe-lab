...
```
(CLI entry files are excluded from coverage; their logic lives in tested `*-main.ts` modules — see Tasks 10–11.)

- [ ] **Step 3: Install and verify**

Run: `npm install && npm run typecheck && npm run lint`
Expected: both succeed (no source files yet is fine).
...
**Files:**
- Create: `scraper/src/coerce.ts`, `scraper/src/slug.ts`, `scraper/src/lab-test-list.ts`, `scraper/src/test-catalogue.ts`
...
describe('parseLabTestList', () => {
  ...
  it('skips rows with empty values or missing urls, keeps first duplicate', () => {
    const json = { headers: ['X', 'Name'], rows: [
    ...
    const rows = parseLabTestList(json, forefootStack);
  ...
  it('throws PayloadError on missing rows or malformed row', () => {
    expect(() => parseLabTestList({ nope: true }, forefootStack)).toThrow(PayloadError);
    expect(() => parseLabTestList({ rows: [['only-one-cell']] }, forefootStack)).toThrow(PayloadError);
  });
});
```
...
function fakeFetch(): typeof fetch {
  return (async (url: RequestInfo | URL) => {
    ...
    if (u.includes('/api/product/lab-test-list/')) {
      ...
      return new Response(labtest5); // same rows for every test id is fine for the test
    }
...
**Interfaces:**
- Consumes: `PoliteHttp`, `HttpStatusError` (5), `extractPagePayload` (4), `extractDetails` (7), `validateDetailsRecord` (8), `DataDir` (10), `buildDataset` (9).
- Produces:
...
Behaviour: targets = `--slug` if given, else all metrics slugs when `forceAll`, else metrics slugs missing from `details.json` (tombstones count as present). Fetch `https://runrepeat.com/uk/{slug}`; `HttpStatusError` 404 → tombstone; other errors → recorded in `failed`, shoe untouched, run continues; run exits non-zero (CLI) if `failed.length > 0` **after** writing successful records. Existing records not in targets are preserved byte-for-byte.
...
- [ ] **Step 4: Run** `npm -w scraper run test -- scrape-details` — Expected: PASS. Then full suite `npm -w scraper run test` — Expected: PASS with coverage thresholds met.
...
export const FLEET: Shoe[] = [
  ...
  shoe({ slug: 'mystery', values: {}, releasedAt: null, score: null }),
];
```
...
describe('stats', () => {
  ...
  it('histogram bins values into range', () => {
    ...
    expect(h.counts[0]).toBe(3); // 0,1,2 fall in [0,2)
...
- Test: `app/src/components/ShoeTable.test.ts`, `app/src/components/DetailPanel.test.ts`
...
- [ ] **Step 2: Run** `npm -w app run test -- ShoeTable DetailPanel` — Expected: FAIL.
...
- [ ] **Step 4: Run** `npm -w app run test -- ShoeTable DetailPanel` — Expected: PASS.
...
jobs:
  ci:
    ...
    steps:
      ...
      - run: npm run lint
      - run: npm run test          # both workspaces, coverage thresholds enforced
