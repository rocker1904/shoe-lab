...
describe('excludedBy', () => {
  it('counts what one bound costs, given the others', () => {
    ...
    const f: FilterState = { categorical: {}, ranges: { weight: { max: 250 }, msrpGbp: { max: 150 } } };
    expect(excludedBy(FLEET, f, 'weight', idx)).toBe(2);
  });

  it('is order-independent — a bound costs the same however the set is written', () => {
    const a: FilterState = { categorical: {}, ranges: { weight: { max: 250 }, msrpGbp: { max: 150 } } };
    const b: FilterState = { categorical: {}, ranges: { msrpGbp: { max: 150 }, weight: { max: 250 } } };
  ...
  it('follows showMissing, because a bound also hides shoes with no reading', () => {
    const f: FilterState = { categorical: {}, ranges: { msrpGbp: { max: 100000 } } };
  ...
  it('is conditioned on the rest of the set, not on the bound alone', () => {
    ...
    const alone: FilterState = { categorical: {}, ranges: { weight: { max: 250 } } };
  ...
  it('costs nothing for a key nothing is bounded on', () => {
    expect(excludedBy(FLEET, { categorical: {}, ranges: { weight: { max: 250 } } }, 'score', idx)).toBe(0);
  });
});
