...
describe('the Easy score columns are side-paired like any other', () => {
  it('follows a side click, so a table cannot mix a heel score with forefoot measurements', () => {
    ...
    expect(projectSide(v, 'forefoot').columns).toEqual([EASY.keys.forefoot, 'forefoot-stack']);
    expect(projectSide(v, 'heel').columns).toEqual([EASY.keys.heel, 'heel-stack']);
  });

  it('follows the sort key too', () => {
    const v: ViewState = { ...withCols([EASY.keys.heel]), sort: { key: EASY.keys.heel, dir: 'desc' } };
    expect(projectSide(v, 'forefoot').sort.key).toBe(EASY.keys.forefoot);
  });

  it('names a side on its own, so a score-only table is not sideless', () => {
    for (const side of SIDES) expect(sideOf(withCols([EASY.keys[side]]))).toBe(side);
    expect(sideOf(withCols([EASY.keys.heel, EASY.keys.forefoot]))).toBeNull();
  });
});
