...
describe('stableConsidered', () => {
  it('is the same population applyFilters reports as considered', () => {
    ...
    const f: FilterState = { ranges: { weight: { max: 250 } }, plate: ['carbon'] };
  ...
  it('keeps its identity when the filters are rebuilt with the same values', () => {
    const read = stableConsidered();
    const first = read(FLEET, { ranges: {}, brands: ['Alpha'], plate: ['carbon'], search: 'cushy' }, idx);
    const second = read(FLEET, { ranges: {}, brands: ['Alpha'], plate: ['carbon'], search: 'cushy' }, idx);
    expect(second).toBe(first);
  });
  ...
  it('rebuilds when a non-range filter moves, in either direction', () => {
    ...
    const carbon = read(FLEET, { ranges: {}, plate: ['carbon'] }, idx);
  ...
  it('gives each reader its own answer, so two populations do not evict each other', () => {
    ...
    const a = mine(FLEET, { ranges: {}, plate: ['carbon'] }, idx);
    const b = yours(FLEET, EMPTY_FILTERS, idx);
    expect(mine(FLEET, { ranges: { weight: { max: 250 } }, plate: ['carbon'] }, idx)).toBe(a);
    expect(yours(FLEET, { ranges: { weight: { max: 250 } } }, idx)).toBe(b);
  });
});

describe('stableBrandCounts', () => {
  it('counts the population with the brand filter itself removed', () => {
    const counts = stableBrandCounts()(FLEET, { ranges: {}, brands: ['Other'] }, idx);
  ...
  it('offers a brand the catalogue no longer carries, so a link can untick it', () => {
    const counts = stableBrandCounts()(FLEET, { ranges: {}, brands: ['Gone'] }, idx);
    expect(counts.get('Gone')).toBe(0);
  });

  it('keeps its identity across a range-bound change', () => {
    const read = stableBrandCounts();
    const first = read(FLEET, { ranges: { weight: { max: 260 } } }, idx);
    expect(read(FLEET, { ranges: { weight: { max: 250 } } }, idx)).toBe(first);
  });

  it('rebuilds when the selection or another filter moves', () => {
    const read = stableBrandCounts();
    const first = read(FLEET, { ranges: {} }, idx);
    expect(read(FLEET, { ranges: {}, brands: ['Other'] }, idx)).not.toBe(first);
    expect(read(FLEET, { ranges: {}, brands: ['Other'], plate: ['carbon'] }, idx)).not.toBe(first);
  });
});
