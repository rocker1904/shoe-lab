...
<style>
   ...
   *
   * Its footprint is `--caret-w` — the glyph plus the air beside it — declared as a token rather
   * than as a margin here, because `fit.ts` has to state the same width as arithmetic when it
   * models a header's minimum, and a number owned by two files drifts
   * (docs/app.md §Table presentation). `flex: none` so a tight header cell squashes the name rather
   * than the mark.
   *
   * `justify-content: flex-end` puts the glyph at the trailing end of its own box, so the air falls
   * on the other side. That reads correctly whichever end of the name the box sits at: after the
   * name in a phrase column the air separates the two, and BEFORE the name in a figure column —
   * where the mark leads, so the label can keep the edge its figures keep — the glyph hugs the
   * first letter and the air falls away from it.
   */
