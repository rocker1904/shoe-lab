<script lang="ts">
  ...
  let { side, onchange }: {
    ...
    side: Side | null; onchange: (s: Side) => void;
  } = $props();

  // A peer of the story pills, in the toolbar: the side applies whether or not a story is
...
<span class="side" role="radiogroup" aria-label="Measurements from" use:roving>
  {#each SIDES as s (s.v)}
    <button type="button" role="radio" aria-checked={side === s.v} class:on={side === s.v}
            onclick={() => onchange(s.v)}>{s.label}</button>
  {/each}
</span>

<style>
  .side { display: inline-flex; border: 1px solid var(--border); border-radius: var(--r-full); overflow: hidden; }
