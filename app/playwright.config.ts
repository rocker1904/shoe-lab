...
export default defineConfig({
   ...
   * those quantified contracts, retired-method surface bounds, the CSP and the column-guide path
   * run in all three engines. `features.spec.ts` runs in all three for disclosure and mounted
   * shared-radio behaviour that jsdom cannot resolve.
   */
  projects: [
    ...
    { name: 'chromium-segmented', use: { browserName: 'chromium' }, testMatch: /cross-browser/,
      grep: /content security policy|holds every shared segment|holds every segment and generation choice|holds the complete touch registry|holds retired method labels|column picker guide/ },
