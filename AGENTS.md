...
- **Commands**: root `npm run verify` is the CI gate (check:docs +
  check:workflows + typecheck + lint + test:coverage — the sequence has one
  owner; run it before pushing)
...
- **Commits**: concise single-line subjects, no embedded measurements. Do
  not hard-code a provider identity: when the user or active environment
  requires agent attribution, use that environment's standard commit
  metadata and name the model that actually authored the change.
- **Comments are WHY-only** — docs/README.md §Rules, rule 6.
- **Design and delivery use the local skills** `designing` and `delivering`
  (`.agents/skills/`). Provider-specific entry points may delegate to those
  canonical files, but do not maintain a second copy. Specs live in
  `docs/specs/`, frozen when superseded.
